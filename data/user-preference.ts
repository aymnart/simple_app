...
import { isFontType,defaultFont } from "@/font.config";
import { db } from "@/lib/db";
import type {  Theme } from "@/themes.config";
import { isThemeType,defaultTheme } from "@/themes.config";
...
>(
  ...
  try {
    const prefs: Partial<{ theme: Theme; font: Font } | null> =
      await db.userPreference.findFirst({
        where: {
          userId,
